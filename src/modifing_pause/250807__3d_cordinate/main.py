...
POSITIONS = utils.generate_points_with_ylim(NUMBER_UE, SATELLITE_R - 100, 0, 0, 0, ylim)
...
def global_stats_collector_draw_middle(env, UEs, satellites, timestep):
    while True:
        ...
        for ue_id in UEs:
            ue = UEs[ue_id]
            pos = (ue.position_x, ue.position_y, ue.position_z)
        ...
        for s_id in satellites:
            s = satellites[s_id]
            satellite_positions.append((s.position_x, s.position_y, s.position_z))
...
for index, position in enumerate(POSITIONS, start=1):
    ...
    for sat_id, sat in satellites.items():
        dist = math.dist(position, (sat.position_x, sat.position_y, sat.position_z))
        if dist < min_dist:
            min_dist = dist
